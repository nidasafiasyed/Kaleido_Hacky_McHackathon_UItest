...
class GovtContract {
    ...
    compile() {
        ...
        if (!this.compiled) {
            ...
            let filePath = path.resolve(__dirname, 'contracts', 'GovtContract.sol');
            ...
            this.bytecode = "0x" + compiled.contracts[':govtcontract'].bytecode;

            // Store the abi (application binary interface) for js to interface with the contract on the chain
            let contractInterface = compiled.contracts[':govtcontract'].interface; // this is a json object that web3 uses as an interface
    ...
    set(amtValue,depValue) {
        // Change json string to int value
        let amtval = parseInt(amtValue);
        let depval = parseInt(depValue);
        ...
        return this.node1.then((web3) => {
            ...
            return web3.eth.getAccounts().then(accounts => {
                ...
                return this.contractInterface.methods.set(amtval,depval).estimateGas({from: defaultAccount}).then((gasAmount) => {
                    console.log("Sending the transaction to set a value");
                    // Send the transaction with the estimated gas
                    return this.contractInterface.methods.set(amtval,depval).send({from: defaultAccount, gas: gasAmount});
                });
            });
        });
    }
